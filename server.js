 ...
 * @param {string} ext - File extension (emf or wmf)
 * @returns {Buffer|null} - PNG buffer or null if conversion fails
 */
function convertEmfWmfToPng(buffer, ext) {
  ...
  try {
    ...
    // Try LibreOffice first (most reliable for EMF/WMF)
    try {
      execSync(
...
function rubyConvertOleBinToMathML(oleBinBuffer, filenameForTmp) {
  ...
  try {
    const out = execFileSync(
      "ruby",
      [path.join(process.cwd(), "mt2mml.rb"), tmpPath],
      { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
    );
    const mathml = (out || "").trim();
...
/**
 * CRITICAL FIX: Render paragraph with correct image/text ordering
 * Images MUST be rendered in their exact position within the run,
...
function renderParagraph(pNode, ctx) {
  ...
  for (const rWrap of runs) {
    ...
    // ✅ FIX: Process children in order to maintain image/text position
    // Instead of collecting all text then all images, we process each element in order
    ...
    if (Array.isArray(rNode)) {
      for (const child of rNode) {
        ...
        // Handle drawing (images) - render immediately at this position
        ...
        // Handle pict (legacy images/objects)
      ...
      // Fallback for non-array rNode (shouldn't happen with preserveOrder, but just in case)
    ...
    // ✅ Also handle images at run level (for nested structures)
    // This catches any images that weren't processed in the child loop
    const runImgRids = unique(findImageEmbedRidsDeep(rNode, []));
    const processedInLoop = new Set();
    
    // Mark which rids we already processed
    if (Array.isArray(rNode)) {
      for (const child of rNode) {
    ...
    // Process any remaining image rids not handled in the loop
    ...
    // ✅ Handle OLE math objects at run level (same as images above)
    if (runHasOleLike(rNode)) {
      debug.seenOleRuns++;
      const rids = unique(findAllRidsDeep(rNode, []));
      
      // Check which math rids we already processed
...
function findSolutionMarkerIndex(html, fromIndex = 0) {
  const s = String(html || "");
  // Regex này tìm từ khóa "Lời giải" ngay cả khi có các thẻ HTML <span> hoặc <strong> ở giữa
...
function splitSolutionSections(tailHtml) {
  ...
  // Tìm vị trí của "Giải chi tiết" để tách làm đôi
  ...
  if (matchCT) {
    ...
    return {
      ...
      detailHtml: s.slice(idxCT).trim(), // Giữ nguyên HTML để bảo tồn thẻ <img>
    };
  }
  
  // Nếu không có phần "Giải chi tiết" riêng, toàn bộ là Lời giải
...
/**
 * ✅ FIX: Only remove truly broken images, NOT valid EMF/WMF/PNG/JPEG
 * EMF/WMF should already be converted to PNG at this point
 */
function removeUnsupportedImages(html) {
  ...
  // 2. No src attribute at all
  s = s.replace(/<img(?![^>]*src\s*=)[^>]*>/gi, "");
  
  // 3. application/octet-stream (usually broken)
  s = s.replace(/<img[^>]*data:application\/octet-stream[^>]*>/gi, "");
  
  // ❌ REMOVED: Don't remove EMF/WMF - they should be converted to PNG already
  // ❌ REMOVED: Don't remove images with short base64 - they might be valid small images
...
function splitChoicesHtmlABCD(blockHtml) {
  ...
  for (let i = 0; i < hits.length; i++) {
    ...
    // Clean images only from individual choices
...
function splitStatementsHtmlabcd(blockHtml) {
  ...
  for (let i = 0; i < hits.length; i++) {
    ...
    // Clean images only from individual statements
    out[key] = removeUnsupportedImages(seg.trim());
  }
  return out;
}
...
function parseExamFromInlineHtml(inlineHtml) {
  ...
  for (const b of blocks) {
    ...
    // Tìm đoạn này trong hàm parseExamFromInlineHtml(inlineHtml) của bạn và thay thế:

    const solIdx = findSolutionMarkerIndex(b.html, 0);
    // Tách phần thân (Stem) và phần đuôi chứa lời giải (Tail)
    const stemPart = solIdx >= 0 ? b.html.slice(0, solIdx).trim() : b.html;
    const tailPart = solIdx >= 0 ? b.html.slice(solIdx).trim() : "";

    // Sử dụng hàm split để bóc tách Lời giải và Giải chi tiết
    // Đảm bảo hàm này không loại bỏ các thẻ <img>
    ...
    exam.questions.push({
      ...
      boxes: 4,
      // Nếu không tìm thấy cấu trúc 'Lời giải' cụ thể, vẫn giữ nguyên tailPart để tránh mất hình
...
app.post("/convert-docx-html", upload.single("file"), async (req, res) => {
  try {
    ...
    // ✅ FIX: Convert EMF/WMF to PNG before storing
    for (const [rid, mediaPath] of Object.entries(mediaRelMap)) {
      ...
      // Check if EMF or WMF - need conversion
      if (ext === "emf" || ext === "wmf") {
    ...
    const debug = {
      ...
      imagesOk,
      imagesConverted,  // ✅ NEW: Track converted EMF/WMF
      imagesInjected: 0,
    ...
    // ✅ Safe: Only removes truly broken images now
    ...
    return res.json({ ok: true, inlineHtml, exam, debug });
  } catch (e) {
